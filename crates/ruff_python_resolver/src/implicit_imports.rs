...
impl ImplicitImports {
    ...
    pub fn find(dir_path: &Path, exclusions: &[&Path]) -> io::Result<Self> {
    ...
    pub fn filter(&self, imported_symbols: &[String]) -> Option<Self> {
    ...
    /// module descriptor.
    pub fn resolves_namespace_package(&self, imported_symbols: &[String]) -> bool {
    ...
    pub fn get(&self, name: &str) -> Option<&ImplicitImport> {
        self.0.get(name)
    }
}
...
pub struct ImplicitImport {
    ...
    pub py_typed: Option<py_typed::PyTypedInfo>,
}

impl IntoIterator for ImplicitImports {
    type Item = (String, ImplicitImport);
    type IntoIter = IntoIter<String, ImplicitImport>;
