...
use python_ast::{
    self as ast, BoolOp, CmpOp, ContextExpr, ConversionFlag, Expr, FStringElement, FStringElements,
...
impl<'src> Parser<'src> {
    ...
    /// field will be [`ContextExpr::Invalid`].
    ///
    /// See: <https://docs.python.org/3/reference/expressions.html#atom-identifiers>
    pub(super) fn parse_name(&mut self) -> ast::NameExpr {
    ...
    fn parse_atom(&mut self) -> ParsedExpr {
        ...
        let lhs = match self.current_token_kind() {
            ...
            TokenKind::True => {
                ...
                Expr::BooleanLiteral(ast::BooleanLiteralExpr {
                    value: true,
                    range: self.node_range(start),
                })
            }
            ...
            TokenKind::None => {
                ...
                Expr::NoneLiteral(ast::NoneLiteralExpr {
                    range: self.node_range(start),
                })
            }
            TokenKind::Ellipsis => {
                self.bump(TokenKind::Ellipsis);
                Expr::EllipsisLiteral(ast::EllipsisLiteralExpr {
                    range: self.node_range(start),
                })
            }
    ...
    fn parse_call_expression(&mut self, func: Expr, start: TextSize) -> ast::CallExpr {
    ...
    fn parse_subscript_expression(
        ...
        if self.eat(TokenKind::Rsqb) {
            ...
            return ast::SubscriptExpr {
                value: Box::new(value),
                slice: Box::new(Expr::Name(ast::NameExpr {
                    range: slice_range,
                    id: Name::empty(),
                    ctx: ContextExpr::Invalid,
                })),
    ...
    pub(super) fn parse_strings(&mut self) -> Expr {
        ...
        match strings.len() {
            ...
            1 => match strings.pop().unwrap() {
                StringType::Str(string) => Expr::StringLiteral(ast::StringLiteralExpr {
                    value: ast::StringLiteralValue::single(string),
                    range,
                }),
                StringType::Bytes(bytes) => Expr::BytesLiteral(ast::BytesLiteralExpr {
                    value: ast::BytesLiteralValue::single(bytes),
                    range,
                }),
                StringType::FString(fstring) => Expr::FString(ast::FStringExpr {
    ...
    fn handle_implicitly_concatenated_strings(
        ...
        if has_bytes {
            match byte_literal_count.cmp(&strings.len()) {
                ...
                Ordering::Equal => {
                    ...
                    return Expr::from(ast::BytesLiteralExpr {
        ...
        if !has_fstring {
            ...
            return Expr::from(ast::StringLiteralExpr {
    ...
    fn parse_parenthesized_expression(&mut self) -> ParsedExpr {
        ...
        if self.eat(TokenKind::Rpar) {
            return Expr::Tuple(ast::TupleExpr {
                elts: vec![],
                ctx: ContextExpr::Load,
                range: self.node_range(start),
                parenthesized: true,
    ...
    ///
    /// See: <https://docs.python.org/3/reference/expressions.html#list-displays>
    fn parse_list_expression(&mut self, first_element: Expr, start: TextSize) -> ast::ListExpr {
        if !self.at_sequence_end() {
            self.expect(TokenKind::Comma);
        }
    ...
    fn parse_set_expression(&mut self, first_element: Expr, start: TextSize) -> ast::SetExpr {
        if !self.at_sequence_end() {
            self.expect(TokenKind::Comma);
        }
    ...
    ///
    /// [Python grammar]: https://docs.python.org/3/reference/grammar.html
    fn parse_starred_expression(&mut self, context: ExpressionContext) -> ast::StarredExpr {
        let start = self.node_start();
    ...
    fn parse_await_expression(&mut self) -> ast::AwaitExpr {
        let start = self.node_start();
    ...
    fn parse_lambda_expr(&mut self) -> ast::LambdaExpr {
        let start = self.node_start();
    ...
    pub(super) fn parse_if_expression(&mut self, body: Expr, start: TextSize) -> ast::IfExpr {
    ...
    fn parse_ipython_escape_command_expression(&mut self) -> ast::IpyEscapeCommandExpr {
        let start = self.node_start();
        ...
        let command = ast::IpyEscapeCommandExpr {
            range: self.node_range(start),
            kind,
            value,
        };
