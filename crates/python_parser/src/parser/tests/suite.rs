...
mod tests {
    ...
    fn test_parse_tuples() {
        ...
        insta::assert_debug_snapshot!(parse_suite(source));
    }
...
def func[T, U: str, *Ts, **P]():
  ...
  ";
    ...
    fn test_parse_dict_comprehension() {
        ...
        let parse_ast = parse_expression(source);
        insta::assert_debug_snapshot!(parse_ast);
    }
...
";
    ...
    fn test_with_statement_invalid() {
        for source in [
            ...
            assert!(!parse_suite(source).1.is_empty());
        }
    }
...
";
    ...
    fn test_dict_unpacking() {
        let parse_ast = parse_expression(r#"{"a": "b", **c, "d": "e"}"#);
        insta::assert_debug_snapshot!(parse_ast);
    }
    ...
    fn test_modes() {
        ...
        assert!(parse(source, Mode::Expression).parse_errors.is_empty());
        assert!(parse(source, Mode::Module).parse_errors.is_empty());
    }
...
";
    ...
    fn test_invalid_type() {
        assert!(!parse_suite("a: type X = int").1.is_empty());
        assert!(!parse_suite("lambda: type X = int").1.is_empty());
    }
...
y = 100(no)
";
        assert_debug_snapshot!(parse_suite(source));
    }
...
%timeit a == 1
    "
        ...
        let mut parse_err = parse_tokens(lxr, source, Mode::Module).parse_errors;
        assert_eq!(
            parse_err.pop().unwrap().to_string(),
...
"#
    ...
    fn test_unicode_aliases() {
        ...
        let parse_ast = parse_suite(r#"x = "\N{BACKSPACE}another cool trick""#);
        insta::assert_debug_snapshot!(parse_ast);
    }
}
