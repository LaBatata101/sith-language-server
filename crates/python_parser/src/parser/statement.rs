...
impl<'src> Parser<'src> {
    ...
    fn parse_delete_statement(&mut self) -> ast::DeleteStmt {
        let start = self.node_start();
        ...
        let targets = self.parse_comma_separated_list_into_vec(
            ...
            |parser| {
                ...
                helpers::set_expr_ctx(&mut target.expr, ContextExpr::Del);
    ...
    fn parse_return_statement(&mut self) -> ast::ReturnStmt {
        let start = self.node_start();
    ...
    fn parse_raise_statement(&mut self) -> ast::RaiseStmt {
        let start = self.node_start();
    ...
    fn parse_import_statement(&mut self) -> ast::ImportStmt {
        let start = self.node_start();
    ...
    fn parse_from_import_statement(&mut self) -> ast::ImportFromStmt {
        let start = self.node_start();
    ...
    fn parse_pass_statement(&mut self) -> ast::PassStmt {
        let start = self.node_start();
    ...
    fn parse_continue_statement(&mut self) -> ast::ContinueStmt {
        let start = self.node_start();
    ...
    fn parse_break_statement(&mut self) -> ast::BreakStmt {
        let start = self.node_start();
    ...
    fn parse_assert_statement(&mut self) -> ast::AssertStmt {
        let start = self.node_start();
    ...
    fn parse_global_statement(&mut self) -> ast::GlobalStmt {
        let start = self.node_start();
    ...
    fn parse_nonlocal_statement(&mut self) -> ast::NonlocalStmt {
        let start = self.node_start();
    ...
    fn parse_type_alias_statement(&mut self) -> ast::TypeAliasStmt {
        let start = self.node_start();
        self.bump(TokenKind::Type);

        let mut name = Expr::Name(self.parse_name());
        helpers::set_expr_ctx(&mut name, ContextExpr::Store);
    ...
    fn parse_ipython_escape_command_statement(&mut self) -> ast::IpyEscapeCommandStmt {
        let start = self.node_start();
        ...
        ast::IpyEscapeCommandStmt { range, kind, value }
    }
        ...
        fn unparse_expr(parser: &mut Parser, expr: &Expr, buffer: &mut String) {
            match expr {
                ...
                Expr::Subscript(ast::SubscriptExpr { value, slice, .. }) => {
                    ...
                    if let Expr::NumberLiteral(ast::NumberLiteralExpr {
    ...
    fn parse_assign_statement(&mut self, target: ParsedExpr, start: TextSize) -> ast::AssignStmt {
        ...
        for target in &mut targets {
            helpers::set_expr_ctx(target, ContextExpr::Store);
    ...
    fn parse_annotated_assignment_statement(
        ...
        helpers::set_expr_ctx(&mut target.expr, ContextExpr::Store);
    ...
    fn parse_if_statement(&mut self) -> ast::IfStmt {
        let start = self.node_start();
    ...
    fn parse_try_statement(&mut self) -> ast::TryStmt {
    ...
    fn parse_for_statement(&mut self, start: TextSize) -> ast::ForStmt {
    ...
    fn parse_while_statement(&mut self) -> ast::WhileStmt {
        let start = self.node_start();
    ...
    fn parse_with_statement(&mut self, start: TextSize) -> ast::WithStmt {
    ...
    fn try_parse_match_statement(&mut self) -> Option<ast::MatchStmt> {
        let checkpoint = self.checkpoint();
    ...
    ///
    /// See: <https://docs.python.org/3/reference/compound_stmts.html#the-match-statement>
    fn parse_match_statement(&mut self) -> ast::MatchStmt {
        let start = self.node_start();
        self.bump(TokenKind::Match);
    ...
    fn parse_async_statement(&mut self) -> Stmt {
        ...
        match self.current_token_kind() {
            ...
            // async def foo(): ...
            TokenKind::Def => Stmt::FunctionDef(ast::FunctionDefStmt {
                is_async: true,
            ...
            // async with item: ...
            TokenKind::With => Stmt::With(ast::WithStmt {
                is_async: true,
            ...
            TokenKind::For => Stmt::For(ast::ForStmt {
                is_async: true,
    ...
    pub(super) fn validate_assignment_target(&mut self, expr: &Expr) {
        match expr {
            Expr::Starred(ast::StarredExpr { value, .. }) => self.validate_assignment_target(value),
            Expr::List(ast::ListExpr { elts, .. }) | Expr::Tuple(ast::TupleExpr { elts, .. }) => {
                for expr in elts {
