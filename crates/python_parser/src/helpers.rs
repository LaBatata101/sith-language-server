...
use ast::CmpOp;
use python_ast::{self as ast, ContextExpr, Expr};
...
pub(crate) fn set_expr_ctx(expr: &mut Expr, new_ctx: ContextExpr) {
    match expr {
...
pub(crate) fn is_valid_assignment_target(expr: &Expr) -> bool {
    match expr {
        Expr::Starred(ast::StarredExpr { value, .. }) => is_valid_assignment_target(value),
