...
use python_ast::{self as ast, Expr};
...
pub(crate) fn concatenated_strings(
    ...
    if has_bytes {
        ...
        return Ok(Expr::from(ast::BytesLiteralExpr {
    ...
    if !has_fstring {
        ...
        return Ok(Expr::from(ast::StringLiteralExpr {
...
mod tests {
    ...
    fn test_parse_fstring() {
        ...
        let parse_ast = parse_suite(source).0;
    ...
    fn test_parse_empty_fstring() {
        insta::assert_debug_snapshot!(parse_suite(r#"f"""#,).0);
    }
    ...
    fn parse_fstring_error(source: &str) -> FStringErrorType {
        let (_, mut errors) = parse_suite(source);
        errors.reverse();
        match errors.pop().expect("Expected error").error() {
    ...
    fn test_parse_invalid_fstring() {
        ...
        assert!(!parse_suite(r#"f"{class}""#,).1.is_empty());
    }
