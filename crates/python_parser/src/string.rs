...
use python_ast::{self as ast, AnyStringFlags, Expr, StringFlags};
...
#[cfg(test)]
mod tests {
    use python_ast::Suite;

    use crate::error::LexicalErrorType;
    use crate::{parse_module, FStringErrorType, ParseError, ParseErrorType};
    ...
    fn parse_suite(source: &str) -> Result<Suite, ParseError> {
        ...
        if parsed.is_valid() {
            ...
            Err(parsed.into_errors().into_iter().next().unwrap())
        }
    }
