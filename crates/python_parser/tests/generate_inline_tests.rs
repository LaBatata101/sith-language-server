//! This module takes specially formatted comments from `python_parser` code
...
fn generate_inline_tests() -> Result<()> {
    let parser_dir = project_root().join("crates/python_parser/src/parser");
    let tests = TestCollection::try_from(parser_dir.as_path())?;

    let mut test_files = TestFiles::default();
    test_files += install_tests(&tests.ok, "crates/python_parser/resources/inline/ok")?;
    test_files += install_tests(&tests.err, "crates/python_parser/resources/inline/err")?;
