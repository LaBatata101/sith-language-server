...
pub fn walk_stmt<'a, V: StatementVisitor<'a> + ?Sized>(visitor: &mut V, stmt: &'a Stmt) {
    match stmt {
        Stmt::FunctionDef(ast::FunctionDefStmt { body, .. }) => {
            visitor.visit_body(body);
        }
        Stmt::For(ast::ForStmt { body, orelse, .. }) => {
            visitor.visit_body(body);
            visitor.visit_body(orelse);
        }
        Stmt::ClassDef(ast::ClassDefStmt { body, .. }) => {
            visitor.visit_body(body);
        }
        Stmt::While(ast::WhileStmt { body, orelse, .. }) => {
            visitor.visit_body(body);
            visitor.visit_body(orelse);
        }
        Stmt::If(ast::IfStmt {
        ...
        Stmt::With(ast::WithStmt { body, .. }) => {
            visitor.visit_body(body);
        }
        Stmt::Match(ast::MatchStmt { cases, .. }) => {
