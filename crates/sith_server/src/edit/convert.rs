//! Convert between LSP types and Sith-LSP types.
...
impl ToLocation for TextRange {
    fn to_location(
        ...
        let range = self.to_range(text, index, encoding);
        types::Location::new(uri, range)
    }
}

pub fn position_to_offset(contents: &str, position: &types::Position, index: &LineIndex) -> u32 {
    index
        .line_start(
