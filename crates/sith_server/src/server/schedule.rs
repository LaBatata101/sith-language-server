...
pub(crate) fn event_loop_thread(
    ...
    const MAIN_THREAD_NAME: &str = "sith:main";
    Ok(
...
pub(crate) struct Scheduler<'scheduler> {
...
impl<'scheduler> Scheduler<'scheduler> {
    pub(super) fn new(
        session: &'scheduler mut Session,
    ...
    pub(super) fn request<R>(
        &mut self,
        params: R::Params,
        response_handler: impl Fn(R::Result) -> Task<'scheduler> + 'static,
    ) -> crate::Result<()>
    where
    ...
    pub(super) fn response(&mut self, response: lsp_server::Response) -> Task<'scheduler> {
