...
bitflags! {
    pub struct FStringContextFlags: u8 {
...
impl FStringContext {
    pub fn new(flags: FStringContextFlags, nesting: u32) -> Self {
        Self {
            flags,
            nesting,
            format_spec_depth: 0,
    ...
    pub const fn triple_quotes(&self) -> Option<&'static str> {
    ...
    /// two curly braces.
    pub const fn is_in_expression(&self, current_nesting: u32) -> bool {
    ...
    pub const fn is_in_format_spec(&self, current_nesting: u32) -> bool {
    ...
    pub fn try_start_format_spec(&mut self, current_nesting: u32) -> bool {
        if self
...
impl FStrings {
    pub fn push(&mut self, context: FStringContext) {
        self.stack.push(context);
    }

    pub fn pop(&mut self) -> Option<FStringContext> {
        self.stack.pop()
    }

    pub fn current(&self) -> Option<&FStringContext> {
        self.stack.last()
    }

    pub fn current_mut(&mut self) -> Option<&mut FStringContext> {
        self.stack.last_mut()
    }
