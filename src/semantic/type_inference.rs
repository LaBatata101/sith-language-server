...
use super::{
    ...
    ScopeId, SemanticModel, Symbol, SymbolId,
};
...
fn resolve_declaration_type(
    ...
    match declaration {
        Declaration::Other(_) => ResolvedType::Unknown,
        Declaration::Import(_) => {
            // TODO: need multi-file analysis for this
            ResolvedType::Unknown
        }
        Declaration::Assign(ast::AssignStmt { value, .. }) => {
            resolve_expr_type_in_scope(value, semantic, scope_id)
        }
        Declaration::AnnAssign(ast::AnnAssignStmt { annotation, .. }) => {
            resolve_expr_type_in_scope(annotation, semantic, scope_id)
        }
        Declaration::ClassDef(ast::ClassDefStmt { name, .. }) => semantic
            .lookup_symbol_in_scope(&name.id, scope_id)
            .map(|symbol_id| ResolvedType::KnownType(PythonType::Class(symbol_id)))
            .unwrap_or(ResolvedType::Unknown),
        Declaration::Parameter(param) => match param {
            ...
            ParamDeclaration::ParamWithDefault(param_with_default) => {
                if let Some(ref expr) = param_with_default.parameter.annotation {
                    resolve_expr_type_in_scope(expr, semantic, scope_id)
                } else if let Some(ref expr) = param_with_default.default {
                    resolve_expr_type_in_scope(expr, semantic, scope_id)
                } else {
                    ResolvedType::AnyType
                }
        ...
        Declaration::FunctionDef(ast::FunctionDefStmt {
            name,
            returns,
            body,
            ..
