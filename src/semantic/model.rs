...
use python_ast::{self as ast, Stmt};
...
use super::{
    ...
    declaration::{Declaration, DeclarationId, Declarations},
    nodes::{NodeId, NodeRef, Nodes},
...
impl<'a> SemanticModel<'a> {
    ...
    pub fn push_declaration(&mut self, declaration: Declaration<'a>) -> DeclarationId {
    ...
    pub fn statement(&self, node_id: NodeId) -> &Stmt {
        self.nodes
            .ancestor_ids(node_id)
            .find_map(|id| self.nodes[id].as_stmt())
    ...
    pub fn enclosing_scope(&self, name: &str, offset: TextSize) -> Option<ScopeId> {
        for (id, scope) in self.scopes.iter().enumerate() {
            if let Some(symbol_id) = scope.get_symbol(name) {
                if let Some(stmt) = self.symbol(symbol_id).statement(self) {
