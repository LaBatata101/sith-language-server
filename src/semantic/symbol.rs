...
impl<'a> Symbol<'a> {
    ...
    pub fn statement<'b>(&'b self, semantic: &'b SemanticModel<'b>) -> Option<&'b Stmt> {
        self.node_id.map(|stmt_id| semantic.statement(stmt_id))
    }

    pub fn declaration<'b>(&self, semantic: &'b SemanticModel<'b>) -> Option<&'b Declaration<'b>> {
        self.declaration_id.map(|id| &semantic.declarations[id])
    }
